...
const [following,serfollowing]=useState([])
  ...
  const getfollowers=async()=>{
   try{
    const res=await axios.get(`http://localhost:4000/getfollowers/${usersdata._id}`)
  ...
  const getmypost=async()=>{
     try{
      setloader(1)
     const res=await axios.get(`http://localhost:4000/getmypost/${usersdata._id}`)
  ...
  const saveprofilepicture=async(e)=>{
    try{
      ...
      setimageloader(1)
      const res=await axios.post('http://localhost:4000/saveprofileimage',filedata,{
        headers:{
